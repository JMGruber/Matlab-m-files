...
startfile=1; endfile=36;
skipfiles=[]; %numbers don't need to be in any particular order; matrix can also be empty
readdir='H:\Windows\Desktop\Maximes SMS DATA TREATED\fcpa pH5.5\FCPa pH552c';
writedir='H:\Windows\Desktop\Maximes SMS DATA TREATED\fcpa pH5.5\FCPa pH552c\results';
...
fwhm=35; 
skewness=0.1;
peak2=700; peak1=675; %typical peak positions for double-band spectra;

% boundary conditions for fitting
xmin=550; xmax=950; % maximum wavelength window considered in all calculations. Set 0 if full range is requireed.
Qthr=30; % if intensity is below this threshold, chances are good for a misfit. 
          %Also used in double-Gaussian fits to identify a single-peak spectrum
deadthr=10; %if the intensity of all spectra is below this threshold, we could consider it dead
minwavint=500; maxwavint=950; %wavelengths considered together with deadthr. Should be within (xmin,xmax)

fwhmdbl=52; %max fwhm of a single peak before the script considers double bands
fwhmblue=40; %used together with skewblue to differentiate between a red tail and an extra red peak
